...
class JPypeClassLoader(ClassLoader):
    ...
    def load_array_class(self, fully_qualified_array_item_class, array_dimension=1):
        return jpype.JArray(self.process_class_name(fully_qualified_array_item_class), array_dimension)

    def process_class_name(self, fully_qualified_class_name):
        wrapper = self.wrapper_for_java_builtin_type.get(fully_qualified_class_name)
