...
if [[ "$1" != *cdh* ]] #standard hadoop distribution
    then
        ...
        if [[ "$1" == 2.2.* ]]
        then 
            export HADOOP_CONF_DIR=$HADOOP_HOME/etc/hadoop; export HADOOP_BIN=$HADOOP_HOME/sbin; export HADOOP_COMMON_LIB_NATIVE_DIR=${HADOOP_HOME}/lib/native; export HADOOP_OPTS="-Djava.library.path=$HADOOP_HOME/lib";else if [[ "$1" != *cdh* ]]; then export HADOOP_CONF_DIR=$HADOOP_HOME/conf; export HADOOP_BIN=$HADOOP_HOME/bin;
