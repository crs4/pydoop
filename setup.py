...
def get_git_commit():
    ...
    if os.path.exists(git_head):
        with open(git_head) as g:
            git_ref = os.path.join(git_root, g.read().strip().split('ref: ')[1])
...
class BuildPydoop(build):
    ...
    user_options.append(
        ('hdfs-core-impl=', None,
         "hdfscore implementation: " + ", ".join(hdfscore.get_supported_implementations()))
    )
    ...
    def finalize_options(self):
        ...
        assert self.hdfs_core_impl in hdfscore.get_supported_implementations(), 'Invalid hdfscore!'
    ...
    def run(self):
        print "HdfsCore implementation: {0}".format(self.hdfs_core_impl)
        ...
        try:
            ...
            # On NFS, if we clean up right away we have issues with nfs handles being
            # still in the directory trees to be deleted.  So, we sleep a bit and then delete
            time.sleep(0.5)
            self.clean_up()
