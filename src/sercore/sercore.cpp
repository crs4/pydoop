...
initsercore(void) {
  ...
  StringInStreamType.tp_new = PyType_GenericNew;
  if (PyType_Ready(&StringInStreamType) < 0) {
    INIT_RETURN(NULL);;
  }
#ifdef PY3
  ...
  Py_INCREF(&StringInStreamType);
  PyModule_AddObject(m, "StringInStream", (PyObject *)&StringInStreamType);
  INIT_RETURN(m);
}
