...
class TestAvroIO(unittest.TestCase):
    ...
    def write_avro_file(self, file_object, schema, rec_creator, n_samples, sync_interval):
    ...
    def test_seekable(self):
        with open(AVRO_DATA, 'wb') as f:
            self.write_avro_file(f, AVRO_USER_SCHEMA, avro_user_record, 
                                 500, 1024)
        with open(AVRO_DATA, 'rb') as f:
            sreader = SeekableDataFileReader(f, DatumReader())
            sm = sreader.sync_marker
            res = [t for t in it.izip(it.imap(lambda _: f.tell(), 
                                              it.repeat(1)), sreader)]
    ...
    def test_avro_writer(self):
        ...
        ctx = FunkyCtx({'mapreduce.task.partition' : 1,
...
def suite():
  ...
  suite.addTest(TestAvroIO('test_seekable'))
  suite.addTest(TestAvroIO('test_avro_reader'))
  suite.addTest(TestAvroIO('test_avro_writer'))
  return suite
